...
/********************* Define functions **************************/
void print_accel_data(int16_t a, int16_t b, int16_t c);
...
static esp_err_t zb_attribute_handler(const esp_zb_zcl_set_attr_value_message_t *message)
{
    ...
    if (message->info.dst_endpoint == HA_ESP_LIGHT_ENDPOINT) {
        if (message->info.cluster == ESP_ZB_ZCL_CLUSTER_ID_ON_OFF) {
            ...
            // uint8_t data[CUSTOM_STRING_MAX_SIZE];
            // data[0] = CUSTOM_STRING_MAX_SIZE - 1;
            ...
            // int16_t b = (data[2] << 8) | data[3];
            // int16_t c = (data[4] << 8) | data[5];

            // print_accel_data(a, b, c);            
        }
    }
    return ret;
}
...
// void print_accel_data(int16_t a, int16_t b, int16_t c) {
//     float x = (a / 16384.0f) * 9.8f;
//     float y = (b / 16384.0f) * 9.8f;
//     float z = (c / 16384.0f) * 9.8f;
//     ESP_LOGI(TAG, "\nAccel: X=%0.2f m/s^2, Y=%0.2f m/s^2, Z=%0.2f m/s^2\n", x, y, z); 
// }
