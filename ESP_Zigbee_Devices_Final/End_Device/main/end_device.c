...
void send_data(packet *data_to_send)
{
  ...
  esp_zb_zcl_custom_cluster_cmd_req_t custom_req = {
    .zcl_basic_cmd = {
      ...
      .dst_endpoint = COORDINATOR_END_POINT,
      .src_endpoint = END_DEVICE_END_POINT,
    },
...
static esp_err_t zb_custom_cluster_handler(const esp_zb_zcl_custom_cluster_command_message_t *message)
{
    ...
    // printf("%lu\n", (uint32_t)message->info.src_address.u.short_addr);
    // if (received_packet.zigbeeAddress != esp_zb_get_pan_id()) {
