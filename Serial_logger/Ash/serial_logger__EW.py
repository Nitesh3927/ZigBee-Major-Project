...
def generate_log_filename(port):
    timestamp = datetime.now().strftime("%H__%M__%S")
    if port == "COM4":
        return f"ESP32_log_{timestamp}.txt"
    
    if port == "COM5":
        return f"H2_SENDER_log_{timestamp}.txt"
    ...
    try:
        with serial.Serial(port, baud_rate, timeout=1) as ser, open(log_filename, 'w') as log_file:
            ...
            while not stop_event.is_set():
                data = ser.readline()
                if data:
                    # Get the current timestamp in seconds and milliseconds format
                    current_time = datetime.now()
                    timestamp = f"{current_time.second}.{int(current_time.microsecond / 1000):03d} sec"  # Format: s.ms sec
                    decoded_data = data.decode('utf-8', errors='ignore').strip()
                    log_entry = f"[{timestamp}] {decoded_data}"
                    
                    # Display in the text widget
                    text_widget.insert(tk.END, f"{log_entry}\n")
