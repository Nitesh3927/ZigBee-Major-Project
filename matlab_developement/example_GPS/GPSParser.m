classdef GPSParser < matlab.System & coder.ExternalDependency
    ...
    properties(Nontunable)
        ...
        %Separator - , (comma)
        Separator = 44;
        %Character preceding checksum - *(asterisk)
        CheckChar = 42;
        %BufferSize
        BufferSize = 80;
        ...
        % Time
        TimeOpt (1, 1) logical = false;
        % Latitude
        LatOpt (1, 1) logical = false;
        % Longitude
        LonOpt (1, 1) logical = false;
        % Number of Satellites Used
        NoSatOpt (1, 1) logical = false;
        % HDOP
        HdopOpt (1, 1) logical = false;
        % Altitude
        AltOpt (1, 1) logical = false;
        % Mean Sea Level
        MslOpt (1, 1) logical = false;
        % Speed
        SpeedOpt (1, 1) logical = false;
        % Track Angle
        TckAngOpt (1, 1) logical = false;
        % Date
        DateOpt (1, 1) logical = false;
    end
    ...
    methods (Access=protected)
        ...
        function setupImpl(obj) 
            if isempty(coder.target)
                ...
                coder.cinclude('include/gps_parser.h');
    ...
    methods (Static)
        ...
        function updateBuildInfo(buildInfo, context)
            if context.isCodeGenTarget('mex')
                ...
                addSourceFiles(buildInfo,'src/gps_parser.cpp', srcDir);
