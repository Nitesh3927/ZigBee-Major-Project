...
#include "MW_I2C.h"  
	...
	static mw_I2C_dev_h mw_I2C_dev[WIRE_INTERFACES_COUNT] = {
        #if defined(ESP_PLATFORM)
        {0,0,&Wire,_RTT_I2C0_SDA_,_RTT_I2C0_SCL_}
        #else
		{0,0,&Wire}
        #endif
...
{ 
		...
		{
            #if defined(ESP_PLATFORM)
            (mw_I2C_dev[I2CModule].I2Cobj)->begin(mw_I2C_dev[I2CModule].sda_pin,mw_I2C_dev[I2CModule].scl_pin);
            #else
...
{
    //No need to close I2C object as other blocks might use the same object to 
	//clear external I2C devices
}
